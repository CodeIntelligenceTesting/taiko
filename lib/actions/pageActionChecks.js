...
const checkIfElementIsNotCovered = async (e) => {
  function isElementAtPointOrChild() {
    ...
    try {
    console.error(`In isElementAtPointOrChild`);
    console.error(this.outerHTML);
    ...
    if (isElementCoveredByAnotherElement) {
      node = nodes[0];
      console.error(`is covered by another element: ${node}`)
    } else {
      node = getDirectParent(nodes, elem);
      console.error(`may be covered by parent: ${node}`)
    }
    const isNotCovered = (
      elem.contains(node) ||
      node.contains(elem) ||
    ...
    // console.error(JSON.stringify(window.getComputedStyle(node)));
    console.error("is covered by");
    console.error(elem.outerHTML);
    // console.error(JSON.stringify(window.getComputedStyle(elem)));
    console.error(`isNotCovered: ${isNotCovered}`)
    return isNotCovered;
    } catch (e) {
  ...
  console.error(`Checking if element ${objectId} is covered`);
...
const checkIfActionable = async (elem, checks) => {
  ...
  for (let check of checks) {
    ...
    if (!actionable) {
      console.error(`Not actionable: ${check.error}`)
      error = check.error;
      break;
    }
  }
...
const waitAndGetActionableElement = async (selector, force, checks = defaultChecks, args = []) => {
  ...
  await waitUntil(
    async () => {
      ...
      console.error(`${description(selector)}matched ${elementsLength} elements`);
      for (let elem of elements) {
        try {
          console.error(`Scrolling to ${description(selector)}`);
          await scrollToElement(elem);
          console.error(`Checking if ${description(selector)} is actionable`);
